...
abstract: |
    Despite growing investments in cycling infrastructure, current planning approaches lack systematic methods to assess or simulate the potential impacts on cycling safety, trip performance and accessibility, besides the typical ex-ante and ex-post cycling volume comparison at each place. The network performance itself is often not assessed.
  This research addresses this gap by integrating geospatial and accessibility analysis to develop a replicable methodology for evaluating cycling networks at different stages.
  A systematic and reproducible methodology was developed to assess changes in safety, trip performance and accessibility in 3 different years, and across 50 cities, based on their existing OpenStreetMap data for each period. 20 thousand OD pairs were generated based on the recently available Global Building Atlas LoD1.
  A panel data modeling was conducted to estimate the elasticities between the investments in different types of cycling infrastructure and trip performances for each city and year, quantifying the impacts of network improvements on potential cycling benefits.
